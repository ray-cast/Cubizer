...
namespace Chunk
{
	...
	public class MagicalVoxelFileLoad : ScriptableObject
	{
		...
		public static bool GetVisiableFaces(ChunkTree map, ChunkTree.ChunkNode<ChunkPosition<System.Byte>, ChunkEntity> it, int chunkSize, ref VisiableFaces faces)
		{
			...
			for (int i = 0; i < voxel.pack.ModelNums; i++)
			{
				...
				foreach (var entity in entities)
				{
					...
					if (data.triangles.GetLength(0) > 0)
					{
						...
						gameObject.AddComponent<MeshRenderer>().material = (Material)Resources.Load("Editor/Materials/VertexColorShading");
